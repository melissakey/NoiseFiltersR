                                     ...
                                     ...)
{
      ...
      for(i in 1:(nrow(x)-1)){
            equalAtt <- integer(0)
            for(j in (i+1):nrow(x)){
                  if(all(x[i,-classColumn]==x[j,-classColumn])){
                        equalAtt <- c(equalAtt,j)
                  }
                  if(length(equalAtt)>0){
                        tab <- table(x[c(i,equalAtt),classColumn])
                        if(max(tab)!=(length(equalAtt)+1)){
                              inconsistencies <- TRUE
                              x[c(i,equalAtt),classColumn] <- names(tab)[nnet::which.is.max(tab)]
                        }
                  }
            }
      }
